...
class CreateAcceptProductMessageOperation extends BaseOperation
{
    ...
    public function handle()
    {
        ...
        $text = __('messages.notification.chatting.user_payed.approve_payment', ['nickname' => $sender->nickname]);

        $message = $this->createMessage($text, $sender);
        $this->updateUserPayedMessage();
        $this->backgroundOperationHandler($text, $text, MessageType::SELLER_APPROVE_PAYMENT, $message);
    }
    ...
    private function updateUserPayedMessage()
    {
        $this->run(UpdateMessageJob::class, [
            ...
            'values' => [
                'confirmed_at' => new Timestamp(new DateTime()),
            ]
        ]);
    }
    ...
    {
        $attributes = [
            'type'        => MessageType::SELLER_APPROVE_PAYMENT,
            'text'        => $text,
            'attachment'  => null,
            'sender'      => [
