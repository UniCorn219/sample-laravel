...
/**
 * Class NotificationBuyerBuyProductJobQueue
 * @package App\Domains\Notification\Jobs
 */
class NotificationBuyerBuyProductJobQueue extends QueueableJob
{
    protected ProductTransactionable $productTransactionAble;

    /**
     * NotificationBuyerBuyProductJobQueue constructor.
    ...
    public function pushNotification($productTransactionAble, $notificationId): bool
    {
        ...
        $title   = trans('messages.notification.safe_payment.buyer_buy_product.title', ['productName' => $product->title]);
        $content = trans('messages.notification.safe_payment.buyer_buy_product.content', ['buyerName' => $buyer->nickname]);

        $options = [
            'type' => NotificationType::SAFE_PAYMENT,
            'data' => [
                'navigate'        => NotificationType::NAVIGATE['PRODUCT_SCREEN'],
                'action_type'     => NotificationAction::BUYER_BUY_PRODUCT,
