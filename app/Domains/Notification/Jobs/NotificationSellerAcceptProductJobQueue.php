...
/**
 * Class NotificationSellerAcceptProductJobQueue
 * @package App\Domains\Notification\Jobs
 */
class NotificationSellerAcceptProductJobQueue extends QueueableJob
{
    protected ProductTransactionable $productTransactionAble;

    /**
     * NotificationSellerAcceptProductJobQueue constructor.
    ...
    public function pushNotification($productTransactionAble): bool
    {
        ...
        $title   = trans('messages.notification.safe_payment.seller_accept.title', ['productName' => $product->title]);
        $content = trans('messages.notification.safe_payment.seller_accept.content', ['sellerName' => $seller->nickname]);

        $options = [
            'type' => NotificationType::SAFE_PAYMENT,
            'data' => [
                'navigate'    => NotificationType::NAVIGATE['PRODUCT_SCREEN'],
                'action_type' => NotificationAction::SELLER_ACCEPT,
